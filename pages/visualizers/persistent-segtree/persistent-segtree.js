...
function updatePersistent(prevNode, l, r, targetIdx, newVal, currentVersion) {
  ...
  if (targetIdx <= mid) {
    ...
    allLinks.push({
      source: newNode.id,
      target: newNode.left.id,
      isShared: false,
      sourceVersion: currentVersion,
    });
    allLinks.push({
      source: newNode.id,
      target: prevNode.right.id,
      isShared: true,
      sourceVersion: currentVersion,
    }); // SHARED EDGE
  } else {
    ...
    allLinks.push({
      source: newNode.id,
      target: prevNode.left.id,
      isShared: true,
      sourceVersion: currentVersion,
    }); // SHARED EDGE
    allLinks.push({
      source: newNode.id,
      target: newNode.right.id,
      isShared: false,
      sourceVersion: currentVersion,
    });
  }
...
function setupD3() {
  ...
  svg = d3.select('#canvasWrapper').append('svg').attr('width', width).attr('height', height);
...
function renderD3DAG() {
  ...
  svg
    .transition()
    .duration(500)
    .call(zoom.transform, d3.zoomIdentity.translate(canvasW / 2 - 250, 20).scale(0.85));
  ...
  // 2. Draw Nodes
  const nodes = g.selectAll('.node').data(allNodes, (d) => d.id);
