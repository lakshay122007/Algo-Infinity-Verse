...
function initDPTable(N) {
  const tw = U('dpTableContent');
  if (N > 7) {
    tw.innerHTML =
      'DP Table is too large (2^N > 128 rows).<br>Decrease N to &le; 7 to view the state memory.';
    tw.className = 'dp-empty-msg';
    return;
  }
  tw.className = '';
  let maxMask = 1 << N;
  let html = '<table class="dp-table"><tr><th>Mask \\ Node</th>';
  for (let i = 0; i < N; i++) html += `<th>${i}</th>`;
  html += '</tr>';
  for (let mask = 1; mask < maxMask; mask += 2) {
    html += `<tr><th>${mask.toString(2).padStart(N, '0')}</th>`;
    for (let i = 0; i < N; i++) {
      html += `<td id="dp_${mask}_${i}">-</td>`;
    }
    html += '</tr>';
  }
  html += '</table>';
...
function* bruteForceGen() {
  ...
  for (let i = 0; i < allPerms.length; i++) {
    ...
    if (cost < bestCost) {
      ...
      bestPath = [...path];
      logMsg(
        `[BruteForce] New best tour found: ${bestPath.join('→')} (Cost: ${bestCost})`,
        'success'
      );
    }
...
function* heldKarpGen() {
  let N = nodes.length;
  let maxMask = 1 << N;
  let dp = Array(maxMask)
    .fill(0)
    .map(() => Array(N).fill(Infinity));
  let parent = Array(maxMask)
    .fill(0)
    .map(() => Array(N).fill(-1));
      ...
      for (let j = 0; j < N; j++) {
        if (!(mask & (1 << j)) && i !== j) {
          ...
          if (ncost < dp[nextMask][j]) {
            ...
            parent[nextMask][j] = i;
            if (renderDP) {
              let cell = U(`dp_${nextMask}_${j}`);
              if (cell) {
                cell.innerText = ncost;
  ...
  U('statHkOps').innerText = hkOps;
  logMsg(
    `[HeldKarp] Reconstructed optimal tour: ${currHkPath.join('→')} (Cost: ${minFinalCost})`,
    'success'
  );
