...
function drawSkip(){
    ...
    for(let i=0; i<nodes.length; i++){
        let x = padX + i*dx;
        ctx.beginPath(); ctx.moveTo(x, padY); ctx.lineTo(x, dims.h - padY); ctx.stroke();
    }
    ...
    for(let i=0; i<skipState.path.length-1; i++){
        ...
        let x1 = padX + i1*dx, y1 = dims.h - padY - l1*dy;
        let x2 = padX + i2*dx, y2 = dims.h - padY - l2*dy;
...
function* chordLookupGen(start, key){
    ...
    logMsg(`[Chord] Key ${key} found at owner Node ${targetNode.id} in ${chordState.hops} hops.`, 'success');
...
function* skipLookupGen(start, key){
    ...
    logMsg(`[SkipGraph] Starting lookup for key ${key} at Node ${start.id} (Level ${skipState.level})`, 'skip');
    yield;
    ...
    logMsg(`[SkipGraph] Key ${key} found at owner Node ${targetNode.id} in ${skipState.hops} hops.`, 'success');
    skipState.done = true;
}
...
function renderFingerTable(n){
    ...
    if(!n){
        ftc.innerHTML = '<p style="color: #64748b; font-size: 0.8rem;">Hover over a node in the Chord ring to view its Finger Table here.</p>';
        return;
    }
...
function animLoop(){
    ...
    if(cAlive || sAlive){
        ...
        isRunning = false;
        updateStatus(`Race finished! Chord: ${chordState.hops} hops. Skip Graph: ${skipState.hops} hops.`, 'done');
    }
}
...
U('sliderKey').addEventListener('input', (e)=>{ U('lblKey').innerText = e.target.value; });
